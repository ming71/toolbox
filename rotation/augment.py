...
class Affine(object):
    ...
    def __call__(self, img, labels, mode=None):
        mask = [True for x in range(len(labels))]
        if random.random() < self.p:
            if mode == 'xywha':
                labels = rbox_2_quad(labels, mode = 'xywha')
                img, labels, mask = random_affine(img, labels, 
    ...
    def __call__(self, img, labels):
        ...
        for i, augmentation in enumerate(self.augmentations):
            ...
            if random.random() < prob:
                if  i != len(self.augmentations)-1:
                    img, labels = augmentation(img, labels, self.mode)
                else:
                    img, labels, mask = augmentation(img, labels, self.mode)
        return img, labels, mask
