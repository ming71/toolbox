...
# 两种模式：rect是两点的xyxy格式； rotate是xywha(其中a顺时针为正，+-0.5pi)
...
if __name__ == "__main__":
    img_path = '/py/datasets/ship/tiny_ships/yolo_ship/single_class/train_imgs'
    label_path = '/py/datasets/ship/tiny_ships/yolo_ship/single_class/train_imgs'
    ...
    if os.path.isdir(img_path) and os.path.isdir(label_path):
        ...
        for iter in iterations:
            ...
            # 选择label的模式
            object_coors = get_yolo_points(os.path.join(label_path,iter[1]), rotate=False)
            if len(object_coors):
    ...
    # for single img
        ...
        # pass
        object_coors = get_DOTA_points(label_path)
