...
class Feature_Enhance(nn.Module):
    ...
    def forward(self, x):
        ...
        attens = [] 
        for fp in x :
            atten = torch.sigmoid(self.channel_att(fp) * self.spatial_att(fp))
            assert self.mode in ['cls', 'reg'], 'Choose right mode for  FE !(cls or reg)'
            if self.mode == 'reg':
                atten = torch.where(atten>0.6, 1-atten, atten)
#                 atten = torch.where(atten>0.5, torch.zeros_like(atten), atten)
            elif self.mode == 'cls':
#                 atten = 0.5 + 0.5 * torch.sin(atten*math.pi-0.5*math.pi)
                atten = torch.where(atten<0.5, torch.zeros_like(atten),atten)
            attens.append(atten)
            out = atten*fp + fp        
            res.append(out)
        return tuple(res),  tuple(attens)   
